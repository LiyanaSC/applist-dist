...
const _hoisted_1$3 = { class: "preserve-display-style" };
const _hoisted_2$3 = {
  key: 0,
  class: "button-background"
};
const {ref: ref$3,computed: computed$1,onMounted: onMounted$1,onUnmounted: onUnmounted$1,nextTick} = await importShared('vue');
...
onMounted$1(() => { // Exécuté quand le composant est monté
  ...
  window.matchMedia('(prefers-color-scheme: dark)'); // Observe le thème système
...
computed$1(() => listStore.creatingList); // Variable réactive pour contrôler l'affichage du formulaire 
...
const sidebar = ref$3(null); // Référence au composant ChooseListTitle pour pouvoir appeler ses méthodes
const goDown = async () => {
...
return (_ctx, _cache) => {
  return (_openBlock$4(), _createElementBlock$4("nav", {
    ref_key: "sidebar",
    ref: sidebar,
    class: "list-sidebar"
  }, [
    ...
    _createElementVNode$4("div", _hoisted_1$3, [
      (!_unref$3(listStore).creatingList)
        ? (_openBlock$4(), _createElementBlock$4("div", _hoisted_2$3, [
            _createElementVNode$4("button", {
              class: "add-list-button super-action-button",
              onClick: _cache[0] || (_cache[0] = $event => (_unref$3(listStore).creatingList = true, goDown()))
            }, _cache[1] || (_cache[1] = [
              _createElementVNode$4("p", { class: "button-text" }, "Créer une nouvelle Liste", -1)
            ]))
          ]))
        : _createCommentVNode$3("", true)
    ])
...
const Sidebar = /*#__PURE__*/_export_sfc(_sfc_main$3, [['__scopeId',"data-v-678a75d9"]]);
